package com.reryde.app.Fragments;
...
import com.reryde.app.Activities.FreeRides;
import com.reryde.app.Activities.MainActivity;
import com.reryde.app.Models.MatrixModel;
import com.reryde.app.RerydeApplication;
import com.squareup.picasso.Picasso;
import com.reryde.app.Activities.CustomGooglePlacesSearch;
import com.reryde.app.Activities.HistoryActivity;
import com.reryde.app.Activities.ShowProfile;
import com.reryde.app.Helper.ConnectionHelper;
import com.reryde.app.Helper.CustomDialog;
import com.reryde.app.Helper.DataParser;
import com.reryde.app.Helper.SharedHelper;
import com.reryde.app.Helper.URLHelper;
import com.reryde.app.Models.CardInfo;
import com.reryde.app.Models.Driver;
import com.reryde.app.Models.PlacePredictions;
import com.reryde.app.R;
import com.reryde.app.Retrofit.ApiInterface;
import com.reryde.app.Retrofit.RetrofitClient;
import com.reryde.app.Utils.MapAnimator;
import com.reryde.app.Utils.MyBoldTextView;
import com.reryde.app.Utils.MyButton;
import com.reryde.app.Utils.MyTextView;
import com.reryde.app.Retrofit.ResponseListener;
...
GoogleMap.OnCameraIdleListener{
    ...
    String Distance_traveled,time_taken;
...
//    private static final String CONFIG_ENVIRONMENT = PayPalConfiguration.ENVIRONMENT_PRODUCTION;
    ...
    //private static final String CONFIG_CLIENT_ID = "AdMZhiBnvbcG2vXV1yczuW0MBo1waNAPGCH6tMjaUsa-32kimhbddx54MCpnpc-ZQNI7xBVvLKhutiVS";
//    private static final String CONFIG_CLIENT_ID = "AXmuW46RFqzkWiuSeKZtj7qazFtfSBUZHG_p495U3oS3sXg_5uqfFS_LeL7-dh_nvWgcQB9qdeVFr8h2";
    private static final String CONFIG_CLIENT_ID = "AdMZhiBnvbcG2vXV1yczuW0MBo1waNAPGCH6tMjaUsa-32kimhbddx54MCpnpc-ZQNI7xBVvLKhutiVS";
    ...
    private static PayPalConfiguration config = new PayPalConfiguration()
            ...
            .merchantName("Reryde")
            .merchantPrivacyPolicyUri(Uri.parse("https://reryde.com/privacy"))
            .merchantUserAgreementUri(Uri.parse("https://reryde.com/legal"));
            ...
            AsyncTask.execute(new Runnable() {
                ...
                public void run() {
                    ...
                    if (providerLocation != null && !providerLocation.isEmpty()) {
                        if(ETA){
                            ETA = false;
                            GetETANearByProvider(String.valueOf(cmPosition.target.latitude), String.valueOf(cmPosition.target.longitude), String.valueOf(sortLocations(providerLocation, cmPosition.target.latitude, cmPosition.target.longitude).getLatitude()), String.valueOf(sortLocations(providerLocation, cmPosition.target.latitude, cmPosition.target.longitude).getLongitude()));
    ...
    public void GetETANearByProvider(final String currentlat, final String currentlng, final String providerLat, final String providerLng) {
        ...
        Call<MatrixModel> call = service.getDistanceAndDuration("METRIC", currentlat + "," + currentlng, providerLat + "," + providerLng);
        System.out.println("enther googleditance api" + call.request().url());
        call.enqueue(new Callback<MatrixModel>() {
            @Override
            public void onResponse(Call<MatrixModel> call, retrofit2.Response<MatrixModel> response) {
                ETA = true;
                if (response.body() != null && response.body().getStatus().equals("OK")) {
                    if (response.body().getRows().get(0).getElements() != null && !response.body().getRows().get(0).getElements().isEmpty()) {
                        if (response.body().getRows().get(0).getElements().get(0).getDuration() != null) {
                            estimation_time.setText(response.body().getRows().get(0).getElements().get(0).getDuration().getText());
                            ETA_Time = response.body().getRows().get(0).getElements().get(0).getDuration().getText();
                        }
                    }
                }
            ...
            @Override
            public void onFailure(Call<MatrixModel> call, Throwable t) {

            }
        });
    }

    public static Location sortLocations(List<Location> locations, final double myLatitude, final double myLongitude) {
    ...
    String isPaid = "", paymentMode = "", strTotalPrice = "", strServiceName = "", strTipAmt = "";
    ...
    ImageView imgMenu, mapfocus, imgBack, shadowBack, free_run;
    RelativeLayout eta_layout;
    View tripLine;
...
//          <!--5. Invoice Layout ...-->
    ...
    MyBoldTextView lblBasePrice, lblDistanceCovered, lblExtraPrice, lblTimeTaken, lblDistancePrice, lblCommision, lblTaxPrice, lblTips, lblPaymentTypeInvoice, lblPaymentChangeInvoice, lblTipsPayment, lblDiscountPrice, lblWalletPrice;
    RadioGroup radio_tips;
    RadioButton amount_zero,amount_two,amount_three,amount_five,custom_amount;
    RadioButton checked;
    ...
    //rating---new
    ImageView picku_drop_image;
    TextView pickup_rating_txt, drop_address_rating;
...
//            <!-- Static marker-->
    ...
    private List<Location> providerLocation = new ArrayList<>();
    AlertDialog alert;
    String previousTag = "";
    //Animation
...
//    MapRipple mapRipple;
    ...
    public View onCreateView(LayoutInflater inflater, final ViewGroup container,
        ...
        new Handler().postDelayed(new Runnable() {
            ...
            public void run() {
                init(rootView);
//                estimation_time = (TextView) rootView.findViewById(R.id.estimation_time);
...
//        <!-- Source and Destination Layout-->
        ...
        free_run = (ImageView) rootView.findViewById(R.id.free_run);
        eta_layout = (RelativeLayout) rootView.findViewById(R.id.eta_layout);
...
//        <!--  Driver and service type Details-->
        ...
        picku_drop_image = rootView.findViewById(R.id.picku_drop_image);
        pickup_rating_txt = rootView.findViewById(R.id.pickup_rating_txt);
        drop_address_rating = rootView.findViewById(R.id.drop_address_rating);
...
//           <!--5. Invoice Layout ...-->
        ...
        lblTips = (MyBoldTextView) rootView.findViewById(R.id.lblTips);
        ...
        lblTipsPayment = (MyBoldTextView) rootView.findViewById(R.id.lblTipsPayment);
        ...
        radio_tips = (RadioGroup) rootView.findViewById(R.id.radio_tips);
        amount_zero = (RadioButton) rootView.findViewById(R.id.amount_zero);
        amount_two = (RadioButton) rootView.findViewById(R.id.amount_two);
        amount_three = (RadioButton) rootView.findViewById(R.id.amount_three);
        amount_five = (RadioButton) rootView.findViewById(R.id.amount_five);
        custom_amount = (RadioButton) rootView.findViewById(R.id.custom_amount);
        checked = (RadioButton) radio_tips.findViewById(radio_tips.getCheckedRadioButtonId());
...
//          <!--6. Rate provider Layout ...-->
        ...
        //invoice-- new
        driver_img = rootView.findViewById(R.id.driver_img);
        pickup_drop_static_img = rootView.findViewById(R.id.pickup_drop_static_img);
        driver_name_txt = rootView.findViewById(R.id.driver_name_txt);
        pickup_address_txt = rootView.findViewById(R.id.pickup_address_txt);
        drop_address_txt = rootView.findViewById(R.id.drop_address_txt);
...
//            <!--Static marker-->
        ...
        datePicker = (DatePicker) rootView.findViewById(R.id.simpledate);
...
//        simpleTimePicker.setMinute(minute);
        simpleTimePicker.setOnTimeChangedListener(new TimePicker.OnTimeChangedListener() {
            ...
            public void onTimeChanged(TimePicker view, int hourOfDay, int minute) {
                ...
                if (hourOfDay > 12) {
                    ...
                    if (hourOfDay < 10) {
                        choosedHour = "0" + hourOfDay;
                    } else {
                        choosedHour = "" + hourOfDay;
                    }
                } else {
                    choosedTimeZone = "AM";
                    if (hourOfDay < 10) {
                ...
                if (minute < 10) {
                    choosedMinute = "0" + minute;
                } else {
                    choosedMinute = "" + minute;
                }
                ...
                if (scheduledDate != "" && scheduledTime != "") {
                    ...
                    if (!DateUtils.isToday(milliseconds)) {
                        date_time_txt.setText(scheduledDate + "-" + choosedTime);
        ...
        datePicker.init(minDate.get(Calendar.YEAR), minDate.get(Calendar.MONTH), minDate.get(Calendar.DAY_OF_MONTH), new DatePicker.OnDateChangedListener() {
            ...
            public void onDateChanged(DatePicker datePicker, int year, int month, int dayOfMonth) {
                ...
                String choosedMonth = "";
                String choosedDate = "";
                String choosedDateFormat = dayOfMonth + "-" + (month + 1) + "-" + year;
                ...
                afterToday = utils.isAfterToday(year, month, dayOfMonth);
                date_time_txt.setText(choosedDate + " " + choosedMonth + " " + year + "-" + scheduledTime);
        ...
        radio_tips.setOnCheckedChangeListener(new RadioGroup.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(RadioGroup radioGroup, int i) {
                ...
                if (checkedRadioButton.getText().toString().equalsIgnoreCase("0") ){
                    amount_zero.setBackgroundColor(getResources().getColor(R.color.colorPrimary));
                    amount_two.setBackgroundColor(getResources().getColor(R.color.done_text_color_dark_normal));
                    amount_three.setBackgroundColor(getResources().getColor(R.color.done_text_color_dark_normal));
                    amount_five.setBackgroundColor(getResources().getColor(R.color.done_text_color_dark_normal));
                    custom_amount.setBackgroundColor(getResources().getColor(R.color.done_text_color_dark_normal));
                    payTips("0",Distance_traveled,time_taken);
                }
                if (checkedRadioButton.getText().toString().equalsIgnoreCase("$2") ){
                    amount_zero.setBackgroundColor(getResources().getColor(R.color.text_color_white));
                    amount_two.setBackgroundColor(getResources().getColor(R.color.colorPrimary));
                    amount_three.setBackgroundColor(getResources().getColor(R.color.text_color_white));
                    amount_five.setBackgroundColor(getResources().getColor(R.color.text_color_white));
                    custom_amount.setBackgroundColor(getResources().getColor(R.color.text_color_white));
                    payTips("2",Distance_traveled,time_taken);
                }
                if (checkedRadioButton.getText().toString().equalsIgnoreCase("$3") ){
                    amount_zero.setBackgroundColor(getResources().getColor(R.color.text_color_white));
                    amount_two.setBackgroundColor(getResources().getColor(R.color.text_color_white));
                    amount_three.setBackgroundColor(getResources().getColor(R.color.colorPrimary));
                    amount_five.setBackgroundColor(getResources().getColor(R.color.text_color_white));
                    custom_amount.setBackgroundColor(getResources().getColor(R.color.text_color_white));
                    payTips("3",Distance_traveled,time_taken);
                }
                if (checkedRadioButton.getText().toString().equalsIgnoreCase("$5") ){
                    amount_zero.setBackgroundColor(getResources().getColor(R.color.text_color_white));
                    amount_two.setBackgroundColor(getResources().getColor(R.color.text_color_white));
                    amount_three.setBackgroundColor(getResources().getColor(R.color.text_color_white));
                    amount_five.setBackgroundColor(getResources().getColor(R.color.colorPrimary));
                    custom_amount.setBackgroundColor(getResources().getColor(R.color.text_color_white));
                    payTips("5",Distance_traveled,time_taken);
                }
                if (checkedRadioButton.getText().toString().equalsIgnoreCase("other") ){
                    ...
                    amount_three.setBackgroundColor(getResources().getColor(R.color.text_color_white));
                    amount_five.setBackgroundColor(getResources().getColor(R.color.text_color_white));
                    custom_amount.setBackgroundColor(getResources().getColor(R.color.colorPrimary));
                    getTipsAmount();
                }
        ...
        free_run.setOnClickListener(new OnClick());
        ...
        lblTipsPayment.setOnClickListener(new OnClick());
        ...
        try {
            ...
            if (location != null && location.getLatitude() != 0 && location.getLongitude() != 0) {
                ...
                if (value == 0) {
                    ...
                    CameraPosition cameraPosition = new CameraPosition.Builder().target(myLocation).zoom(16.65f).build();
                    Log.e("camera_pos", cameraPosition + "");
                    mMap.moveCamera(CameraUpdateFactory.newCameraPosition(cameraPosition));
        ...
        public void onClick(View v) {
            switch (v.getId()) {
                ...
                case R.id.lblPaymentChange:
                    showChooser();
                    break;
                case R.id.lblTipsPayment:
                    getTipsAmount();
                    break;
                case R.id.btnRequestRides:
                    scheduledDate = "";
                    scheduledTime = "";
                    if (lblPaymentType.getText().toString().equalsIgnoreCase("XXXX-XXXX-XXXX-XXXX")){
                        showChooser();
                        Toast.makeText(context, context.getResources().getString(R.string.add_payments), Toast.LENGTH_SHORT).show();
                    }else {
                        ...
                        try {
                            ...
                            if (strPickType.equalsIgnoreCase("source")) {
                                ...
                                if (dest_lat.equalsIgnoreCase("")) {
                                    ...
                                    strPickType = "";
//                                    getServiceList();
                                    getserviceListFromAddress(source_lat,source_lng);
                                ...
                                if (dest_lat.equalsIgnoreCase(source_lat)) {
                                    ...
                                    strPickType = "";
//                                    getServiceList();
                                    getserviceListFromAddress(dest_lat,dest_lng);
                ...
                case R.id.imgMenu:
                    ...
                    AppsFlyerLib.getInstance().trackEvent(context,"af_view_menu",eventValue);
    ...
    public void navigateToShareScreen(String shareUrl) {
        try {
            ...
            sendIntent.putExtra(Intent.EXTRA_TEXT, "Reryde-" + "Mr/Mrs." + name + " would like to share a ride with you at " +
    ...
    void layoutChanges() {
        try {
            ...
            date_time_txt.setText("Please Choose Date And Time");
            ...
            if (flowValue == 0) {
                ...
                eta_layout.setVisibility(View.VISIBLE);
                if (mMap != null) {
                    mMap.clear();
                    stopAnim();
                    setupMap();
                ...
                eta_layout.setVisibility(View.GONE);
...
//                                getServiceList();
                                ...
                                if (cardInfoArrayList.size() > 0) {
                                    ...
                                    lblPaymentType.setText("XXXX-XXXX-XXXX-XXXX");
                                }
//                                getServiceList();
                                getserviceListFromAddress(source_lat,source_lng);
                            }
    ...
    private void getserviceListFromAddress(String source_lat,String source_lng) {
        ...
        try {
            ...
            JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(Request.Method.GET,
                    URLHelper.GET_SERVICE_LIST_FROM_ADDRESS+"?s_latitude="+source_lat+"&s_longitude="+source_lng, new JSONObject(), new Response.Listener<JSONObject>() {
                @Override
                public void onResponse(JSONObject response) {

                    JSONArray JArray = response.optJSONArray("cityprice");
                    ...
                    try {
                        ...
                        if (SharedHelper.getKey(context, "service_type").equalsIgnoreCase("")) {
                            SharedHelper.putKey(context, "service_type", "" + response.optJSONArray("cityprice").optJSONObject(0).optJSONArray("service").optJSONObject(0).optString("id"));
                        }

                        if (SharedHelper.getKey(context, "city_id").equalsIgnoreCase("")){
                            SharedHelper.putKey(context, "city_id", "" + response.optJSONArray("cityprice").optJSONObject(0).optString("city_id"));
                        }
                        ...
                        if (JArray.length() > 0) {
                            currentPostion = 0;
                            ServiceListAdapter1 serviceListAdapter = new ServiceListAdapter1(JArray);
                        ...
                        if ((customDialog != null) && (customDialog.isShowing()))
                            customDialog.dismiss();
                        Toast.makeText(context, "Unfortunately we are unavailable here", Toast.LENGTH_LONG).show();
                        flowValue = 0;
                        layoutChanges();
                    }
                }
...
//        float base = Float.parseFloat(response.optJSONObject(0).optJSONObject("payment").optString("tax"));
    ...
    public void getApproximateFare() {
        try {
            ...
            String constructedURL = URLHelper.ESTIMATED_FARE_DETAILS_API + "" +
                    ...
                    + "&city_id=" + SharedHelper.getKey(context, "city_id");
                    ...
                    try {
                        ...
                        if (!jsonObj.getString("latitude").equalsIgnoreCase("") && !jsonObj.getString("longitude").equalsIgnoreCase("")) {
                            ...
                            if (mHashMap.containsKey(jsonObj.optInt("id"))) {
                                ...
                                if (!jsonObj.optString("id").isEmpty()) {
                                    providerLocation.add(ProviderLocation(jsonObj.optString("id"), new LatLng(proLat, proLng)));
                                }
                                MarkerOptions markerOptions = new MarkerOptions()
                                        .anchor(0.5f, 0.75f)
...
//                            }
    ...
    public Location ProviderLocation(String id, LatLng LatLng) {
    ...
    public void sendRequest() {
        ...
        try {
            ...
            object.put("city_id", SharedHelper.getKey(context, "city_id"));
            ...
            if (SharedHelper.getKey(context, "payment_mode").equals("CASH") || SharedHelper.getKey(context, "payment_mode").equals("PAYPAL")) {
        ...
        RerydeApplication.getInstance().cancelRequestInQueue("send_request");
...
//                    utils.GoToBeginActivity(getActivity());
    ...
    private void showChooser() {
        Intent intent = new Intent(getActivity(), com.reryde.app.Activities.Payment.class);
...
//        if (cardInfo.getLastFour().equals("PayPal") || cardInfo.getLastFour().equals("CASH")) {
...
//        }
    ...
    public void payNowPayPal() {
        PayPalPayment payPalPayment = new PayPalPayment(new BigDecimal(strTotalPrice), "CAD", strServiceName, PayPalPayment.PAYMENT_INTENT_SALE);
            ...
            public void onErrorResponse(VolleyError error) {
                ...
                if (response != null && response.data != null) {
                    try {
                        ...
                        if (response.statusCode == 400 || response.statusCode == 405 || response.statusCode == 500) {
                            try {
                                utils.displayMessage(getView(), getActivity(), errorObj.optString("message"));
                            } catch (Exception e) {
                                utils.displayMessage(getView(), getActivity(), getString(R.string.something_went_wrong));
                            }
                        } else if (response.statusCode == 401) {
                            refreshAccessToken("SEND_REQUEST");
                        } else if (response.statusCode == 422) {
                            ...
                            json = trimMessage(new String(response.data));
                            if (json != "" && json != null) {
                                utils.displayMessage(getView(), getActivity(), json);
                            } else {
                                utils.displayMessage(getView(), getActivity(), getString(R.string.please_try_again));
                            }
                        } else if (response.statusCode == 503) {
                            utils.displayMessage(getView(), getActivity(), getString(R.string.server_down));
                        } else {
                            utils.displayMessage(getView(), getActivity(), getString(R.string.please_try_again));
                        }
    ...
    public void getTipsAmount() {
        try {
            android.support.v7.app.AlertDialog.Builder mobileNoDialog = new android.support.v7.app.AlertDialog.Builder(getActivity());
            LayoutInflater inflater = this.getLayoutInflater();
            final View mobileView = inflater.inflate(R.layout.get_tips_layout, null);
            mobileNoDialog.setView(mobileView);
            mobileNoDialog.setCancelable(true);

            final EditText get_tips_amt = (EditText) mobileView.findViewById(R.id.ed_pay_tips);

            final Button pay_tips_amt = (Button) mobileView.findViewById(R.id.pay_tips_amt);

            final android.support.v7.app.AlertDialog viewDialog = mobileNoDialog.create();

            pay_tips_amt.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    String tipsStr = get_tips_amt.getText().toString();
                    viewDialog.dismiss();
                    payTips(tipsStr,Distance_traveled,time_taken);
                }
            });
    ...
    public void payTips(final String tipsAmount,final String Distance_traveled,final String time_taken) {
        customDialog = new CustomDialog(context);
        customDialog.setCancelable(false);
        if (customDialog != null)
            customDialog.show();
        ...
        try {
            object.put("request_id", SharedHelper.getKey(context, "request_id"));
            object.put("tip", tipsAmount);

        } catch (Exception e) {
            e.printStackTrace();
        }
        JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(Request.Method.POST, URLHelper.PAY_TIP, object, new Response.Listener<JSONObject>() {
            @Override
            public void onResponse(JSONObject response) {
                utils.print("PayTipRequestResponse", response.toString());
                if ((customDialog != null) && (customDialog.isShowing()))
                    customDialog.dismiss();

                strTotalPrice = (Double.parseDouble(strTotalPrice) - (strTipAmt.equalsIgnoreCase("") ? 0 : Double.parseDouble(strTipAmt))) + "";

                strTotalPrice = (Double.parseDouble(strTotalPrice) + (tipsAmount.equalsIgnoreCase("") ? 0 : Double.parseDouble(tipsAmount))) + "";

                float Payable = Float.parseFloat(strTotalPrice);

                strTipAmt = "" + tipsAmount;

//                lblTotalPrice.setText("Your trip was " + SharedHelper.getKey(context, "currency") + "" + strTotalPrice);

                lblTotalPrice.setText("Your trip was("+Distance_traveled+"km,"+time_taken+ "m) " +SharedHelper.getKey(context,"currency")+String.format("%.2f",Payable));

                lblTips.setText(SharedHelper.getKey(context, "currency") + "" + tipsAmount);
            ...
            if (isInternet) {
                final JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(Request.Method.GET,
                    ...
                    public void onResponse(JSONObject response) {
                        ...
                        reqStatus = "";
                        utils.print("Response", " checkstatus response" + response.toString());
                        ...
                        if (response.optJSONArray("data") != null && response.optJSONArray("data").length() > 0) {
                            ...
                            try {
                                ...
                                requestStatusCheckObject = requestStatusCheck.getJSONObject(0);
                                //Driver Detail
...
//                                    CreateMap(requestStatusCheckObject.optJSONObject("provider").optString("latitude"),requestStatusCheckObject.optJSONObject("provider").optString("longitude"));
                                    GetETANearByProvider(current_lat, current_lng , requestStatusCheckObject.optJSONObject("provider").optString("latitude"),requestStatusCheckObject.optJSONObject("provider").optString("longitude"));
                                }

                                if (PreviousStatus.equalsIgnoreCase("ACCEPTED") || PreviousStatus.equalsIgnoreCase("STARTED")){

                                    if (destinationMarker != null){
                                        destinationMarker.remove();


                                        View mark = ((LayoutInflater) getActivity().getSystemService(Context.LAYOUT_INFLATER_SERVICE)).inflate(R.layout.marker, null);

                                        TextView estimat_time = (TextView) mark.findViewById(R.id.estimation_time);
                                        estimat_time.setText(ETA_Time);


                                        LatLng latlng = new LatLng(Double.parseDouble(current_lat), Double.parseDouble(current_lng));
                                        MarkerOptions markerOptions = new MarkerOptions()
                                                .position(latlng).snippet(frmSource.getText().toString())
                                                .title("source").draggable(true)
                                                .icon(BitmapDescriptorFactory.fromBitmap(createDrawableFromView(context, mark)));
                                        marker = mMap.addMarker(markerOptions);
                                    ...
                                    switch (status) {
                                        ...
                                        case "ACCEPTED":
                                            ...
                                            try {
                                                ...
                                                Picasso.with(context).load(URLHelper.base + "storage/" +provider_service.optString("image")).placeholder(R.drawable.car_select).error(R.drawable.car_select).into(imgServiceRequested);
                                            ...
                                            try {
                                               JSONObject provider = requestStatusCheckObject.optJSONObject("provider");
                                                if (requestStatusCheckObject.optJSONObject("payment") != null) {
                                                    ...
                                                    lblTips.setText(SharedHelper.getKey(context, "currency") + "" + payment.optString("tip"));
                                                    strTipAmt = payment.optString("tip");

                                                    Distance_traveled = requestStatusCheckObject.optString("distance");
                                                    time_taken = requestStatusCheckObject.optString("travel_time");
...
//                                                    lblTotalPrice.setText("Your trip was " + SharedHelper.getKey(context, "currency") + "" + payment.optString("payable"));
                                                    lblTotalPrice.setText("Your trip was("+requestStatusCheckObject.optString("distance")+"km,"+requestStatusCheckObject.optString("travel_time")+ "m) " +SharedHelper.getKey(context,"currency")+requestStatusCheckObject.optJSONObject("payment").optString("payable"));

                                                    strTotalPrice = payment.optString("payable");
                                                    ...
                                                    if (isPaid.equalsIgnoreCase("1") && paymentMode.equalsIgnoreCase("CASH")
                                                        ...
                                                        imgPaymentTypeInvoice.setImageResource(R.drawable.visa);
                                                        lblPaymentTypeInvoice.setText("CARD");
                                                    } else if (isPaid.equalsIgnoreCase("1") && paymentMode.equalsIgnoreCase("PAYPAL")
                                                            && walletAmountDetected > 0 && couponAmountDetected > 0 && totalRideAmount == 0) {
                                                        btnPayNow.setVisibility(View.GONE);
                                                        btnPaymentDoneBtn.setVisibility(View.VISIBLE);
                                                        walletDetectionLayout.setVisibility(View.VISIBLE);
                                        ...
                                        case "COMPLETED":
                                            strTag = "";
                                            try {
                                                pickup_rating_txt.setText(requestStatusCheckObject.optString("s_address"));
                                                drop_address_rating.setText(requestStatusCheckObject.optString("d_address"));


                                                Glide.with(activity).load(requestStatusCheckObject.optJSONObject("payment").optString("static_map")).placeholder(R.drawable.placeholder).error(R.drawable.placeholder).into(pickup_drop_static_img);



                                                if (requestStatusCheckObject.optJSONObject("payment") != null) {

                                                    pickup_address_txt.setText(requestStatusCheckObject.optString("s_address"));
                                                    drop_address_txt.setText(requestStatusCheckObject.optString("d_address"));
                                                    strServiceName = requestStatusCheckObject.optJSONObject("service_type").optString("name");
                                                    JSONObject payment = requestStatusCheckObject.optJSONObject("payment");
                                                    JSONObject provider = requestStatusCheckObject.optJSONObject("provider");
                                                    Glide.with(activity).load(payment.optString("static_map")).placeholder(R.drawable.placeholder).error(R.drawable.placeholder).into(pickup_drop_static_img);
                                                    Glide.with(activity).load(requestStatusCheckObject.optJSONObject("payment").optString("static_map")).placeholder(R.drawable.placeholder).error(R.drawable.placeholder).into(picku_drop_image);

                                                    driver_name_txt.setText(provider.optString("first_name")+" "+provider.optString("last_name"));
                                                    if (provider.optString("avatar").startsWith("http"))
                                                        Picasso.with(context).load(provider.optString("avatar")).placeholder(R.drawable.ic_dummy_user).error(R.drawable.ic_dummy_user).into(driver_img);
                                                    else
                                                        Picasso.with(context).load(URLHelper.base + "storage/" + provider.optString("avatar")).placeholder(R.drawable.ic_dummy_user).error(R.drawable.ic_dummy_user).into(driver_img);
                                                    isPaid = requestStatusCheckObject.optString("paid");
                                                    paymentMode = requestStatusCheckObject.optString("payment_mode");
                                                    imgSos.setVisibility(View.GONE);
                                                    totalRideAmount = payment.optInt("payable");
                                                    ...
                                                    lblDistancePrice.setText(SharedHelper.getKey(context, "currency") + ""
                                                            + payment.optString("distance"));
                                                    /*lblTotalPrice.setText("Your trip was " + SharedHelper.getKey(context, "currency") + ""
    ...
    private void CreateMap(String lat, String lng) {
        if (mMap != null) {
            /*if (!source_lat.equalsIgnoreCase("") && !source_lng.equalsIgnoreCase("")) {
                sourceLatLng = new LatLng(Double.parseDouble(source_lat), Double.parseDouble(source_lng));
            }
...
//            sourceLatLng = Double.parseDouble(lat);

                    utils.print("LatLng", "Source:" + sourceLatLng + " Destination: " + destLatLng);
            //String url = getDirectionsUrl(sourceLatLng, destLatLng);
            String url = getUrl(Double.parseDouble(lat), Double.parseDouble(lng), Double.parseDouble(current_lat), Double.parseDouble(current_lng));
            fetchUrl = new FetchUrl();
            fetchUrl.execute(url);
...
//	        }
    ...
    private class ServiceListAdapter extends RecyclerView.Adapter<ServiceListAdapter.MyViewHolder> {
        ...
        public void onBindViewHolder(ServiceListAdapter.MyViewHolder holder, final int position) {
            ...
            if (jsonArray.optJSONObject(currentPostion).optString("capacity").equalsIgnoreCase("1")) {
                holder.serviceTitle.setText(jsonArray.optJSONObject(position).optString("name") + jsonArray.optJSONObject(currentPostion).optString("capacity"));

            } else {
                holder.serviceTitle.setText(jsonArray.optJSONObject(position).optString("name") + "(1-" + jsonArray.optJSONObject(currentPostion).optString("capacity") + ")");

            }
            holder.amount.setText(SharedHelper.getKey(context, "currency") + jsonArray.optJSONObject(position).optString("fixed"));
            if (position == currentPostion) {
                ...
                Glide.with(activity).load(jsonArray.optJSONObject(position).optString("image"))
                        .placeholder(R.drawable.car_select).dontAnimate().skipMemoryCache(false).error(R.drawable.car_select).into(holder.serviceImg);
                ...
                holder.amount.setTextColor(ContextCompat.getColor(context, R.color.text_color_white));
                holder.Model.setTextColor(ContextCompat.getColor(context, R.color.text_color_white));
                ...
                holder.amount.setTextColor(ContextCompat.getColor(context, R.color.black_text_color));
                holder.Model.setTextColor(ContextCompat.getColor(context, R.color.black_text_color));
                LinearLayout.LayoutParams layoutParams = new LinearLayout.LayoutParams
        ...
        public class MyViewHolder extends RecyclerView.ViewHolder {
            ...
            MyTextView serviceTitle, Model, amount;
            ...
            public MyViewHolder(View itemView) {
                ...
                Model = (MyTextView) itemView.findViewById(R.id.Model);
    ...
    private class ServiceListAdapter1 extends RecyclerView.Adapter<ServiceListAdapter1.MyViewHolder1> {
        JSONArray jsonArray;
        private SparseBooleanArray selectedItems;
        int selectedPosition;

        public ServiceListAdapter1(JSONArray array) {
            this.jsonArray = array;
        }


        @Override
        public ServiceListAdapter1.MyViewHolder1 onCreateViewHolder(ViewGroup parent, int viewType) {
            @SuppressLint("InflateParams") View view = LayoutInflater.from(getActivity()).inflate(R.layout.service_type_list_item, null);
            return new ServiceListAdapter1.MyViewHolder1(view);
        }

        @Override
        public void onBindViewHolder(ServiceListAdapter1.MyViewHolder1 holder, final int position) {
            utils.print("Title: ", "" + jsonArray.optJSONObject(position).optJSONArray("service").optJSONObject(0).optString("name") + " Image: " + jsonArray.optJSONObject(position).optJSONArray("service").optJSONObject(0).optString("image"));
            if (jsonArray.optJSONObject(currentPostion).optJSONArray("service").optJSONObject(0).optString("capacity").equalsIgnoreCase("1")) {
                holder.serviceTitle.setText(jsonArray.optJSONObject(position).optJSONArray("service").optJSONObject(0).optString("name"));
                holder.amount.setText( jsonArray.optJSONObject(currentPostion).optJSONArray("service").optJSONObject(0).optString("capacity"));
//                holder.amount.setCompoundDrawablesWithIntrinsicBounds(R.drawable.people, 0, 0, 0);
            } else {
                holder.serviceTitle.setText(jsonArray.optJSONObject(position).optJSONArray("service").optJSONObject(0).optString("name"));
                holder.amount.setText("(1-" + jsonArray.optJSONObject(currentPostion).optJSONArray("service").optJSONObject(0).optString("capacity") + ")");
//                holder.amount.setCompoundDrawablesWithIntrinsicBounds(R.drawable.people, 0, 0, 0);
            }
//            holder.amount.setText(SharedHelper.getKey(context, "currency") + jsonArray.optJSONObject(position).optString("fixed"));
            if (position == currentPostion) {
                SharedHelper.putKey(context, "service_type", "" + jsonArray.optJSONObject(position).optJSONArray("service").optJSONObject(0).optString("id"));
                SharedHelper.putKey(context, "city_id", "" + jsonArray.optJSONObject(position).optString("city_id"));
                Glide.with(activity).load(jsonArray.optJSONObject(position).optJSONArray("service").optJSONObject(0).optString("image"))
                        .placeholder(R.drawable.car_select).dontAnimate().skipMemoryCache(false).error(R.drawable.car_select).into(holder.serviceImg);
                holder.selector_background.setBackgroundResource(R.drawable.full_rounded_button_accent);
                holder.serviceTitle.setTextColor(ContextCompat.getColor(context, R.color.text_color_white));
                holder.amount.setTextColor(ContextCompat.getColor(context, R.color.text_color_white));
                holder.img_People.setColorFilter(getContext().getResources().getColor(R.color.text_color_white));
                holder.Model.setTextColor(ContextCompat.getColor(context, R.color.text_color_white));
                LinearLayout.LayoutParams layoutParams = new LinearLayout.LayoutParams
                        (Math.round(context.getResources().getDimension(R.dimen._50sdp)), Math.round(context.getResources().getDimension(R.dimen._50sdp)));
                holder.serviceImg.setLayoutParams(layoutParams);
                ...
                holder.serviceTitle.setTextColor(ContextCompat.getColor(context, R.color.black_text_color));
                holder.amount.setTextColor(ContextCompat.getColor(context, R.color.black_text_color));
                holder.img_People.setColorFilter(getContext().getResources().getColor(R.color.black_text_color));
                holder.Model.setTextColor(ContextCompat.getColor(context, R.color.black_text_color));
                LinearLayout.LayoutParams layoutParams = new LinearLayout.LayoutParams
                        (Math.round(context.getResources().getDimension(R.dimen._40sdp)), Math.round(context.getResources().getDimension(R.dimen._40sdp)));
                holder.serviceImg.setLayoutParams(layoutParams);
            ...
            holder.linearLayoutOfList.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    if (position == currentPostion) {
                        try {
                            lnrHidePopup.setVisibility(View.VISIBLE);
                            showProviderPopup1(jsonArray.getJSONObject(position));
                        } catch (JSONException e) {
                            e.printStackTrace();
                        }
                    }
                    currentPostion = Integer.parseInt(view.getTag().toString());
                    SharedHelper.putKey(context, "service_type", "" + jsonArray.optJSONObject(Integer.parseInt(view.getTag().toString())).optJSONArray("service").optJSONObject(0).optString("id"));
                    SharedHelper.putKey(context, "city_id", "" + jsonArray.optJSONObject(Integer.parseInt(view.getTag().toString())).optString("city_id"));
                    SharedHelper.putKey(context, "name", "" + jsonArray.optJSONObject(currentPostion).optJSONArray("service").optJSONObject(0).optString("name"));
                    notifyDataSetChanged();
                    utils.print("service_type", "" + SharedHelper.getKey(context, "service_type"));
                    utils.print("Service name", "" + SharedHelper.getKey(context, "name"));
                    getProvidersList(SharedHelper.getKey(context, "service_type"));
        ...
        public class MyViewHolder1 extends RecyclerView.ViewHolder {
            ...
            MyTextView serviceTitle, Model, amount;
            ImageView serviceImg,img_People;
            LinearLayout linearLayoutOfList;
            FrameLayout selector_background;
            ...
            public MyViewHolder1(View itemView) {
                ...
                Model = (MyTextView) itemView.findViewById(R.id.Model);
                amount = (MyTextView) itemView.findViewById(R.id.amount);
                img_People = (ImageView) itemView.findViewById(R.id.img_People);
                serviceImg = (ImageView) itemView.findViewById(R.id.serviceImg);
                linearLayoutOfList = (LinearLayout) itemView.findViewById(R.id.LinearLayoutOfList);
                selector_background = (FrameLayout) itemView.findViewById(R.id.selector_background);
                height = itemView.getHeight();
    ...
    private void showProviderPopup1(JSONObject jsonObject) {

        JSONObject jObject = jsonObject.optJSONArray("service").optJSONObject(0);
        ...
        Glide.with(activity).load(jObject.optString("image")).placeholder(R.drawable.pickup_drop_icon).dontAnimate()
                .error(R.drawable.pickup_drop_icon).into(imgProviderPopup);
        ...
        if (jObject.optString("calculator").equalsIgnoreCase("MIN")
                || jObject.optString("calculator").equalsIgnoreCase("HOUR")) {
            lnrPriceBase.setVisibility(View.VISIBLE);
            lnrPricemin.setVisibility(View.VISIBLE);
            if (jObject.optString("calculator").equalsIgnoreCase("MIN")) {
//                lblCalculationType.setText("Minutes");
...
//                lblCalculationType.setText("Hours");
        ...
        } else if (jObject.optString("calculator").equalsIgnoreCase("DISTANCE")) {
            lnrPriceBase.setVisibility(View.VISIBLE);
            lnrPricekm.setVisibility(View.VISIBLE);
//            lblCalculationType.setText("Distance");
        } else if (jObject.optString("calculator").equalsIgnoreCase("DISTANCEMIN")
                || jObject.optString("calculator").equalsIgnoreCase("DISTANCEHOUR")) {
            lnrPriceBase.setVisibility(View.VISIBLE);
            lnrPricemin.setVisibility(View.VISIBLE);
            lnrPricekm.setVisibility(View.VISIBLE);
            if (jObject.optString("calculator").equalsIgnoreCase("DISTANCEMIN")) {
//                lblCalculationType.setText("Distance and Minutes");
            } else {
//                lblCalculationType.setText("Distance and Hours");
            }
        ...
        if (!jObject.optString("capacity").equalsIgnoreCase("null")) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                lblCapacity.setText(jObject.optString("capacity"));
            } else {
                lblCapacity.setText(jObject.optString("capacity") + " peoples");
            }
        } else {
            lblCapacity.setVisibility(View.GONE);
        }

        lblServiceName.setText("" + jObject.optString("name"));

        float base = Float.parseFloat(jsonObject.optString("fixed"));
        float Km = Float.parseFloat(jsonObject.optString("price"));
        float Min = Float.parseFloat(jsonObject.optString("minute"));
        float Mini = Float.parseFloat(jObject.optString("minimum_charge"));



        lblBasePricePopup.setText(SharedHelper.getKey(context, "currency") + String.format("%.2f",base));
        lblPriceKm.setText(SharedHelper.getKey(context, "currency") + String.format("%.2f",Km));
        lblPriceMin.setText(SharedHelper.getKey(context, "currency") + String.format("%.2f",Min));

        if (!jObject.optString("minimum_charge").equalsIgnoreCase("null")) {
            lblCalculationType.setText(SharedHelper.getKey(context, "currency") + String.format("%.2f",Mini));
        }else {
            lblCalculationType.setText(SharedHelper.getKey(context, "currency") + "0.00");
        }

        if (jObject.optString("description").equalsIgnoreCase("null")) {
            lblProviderDesc.setVisibility(View.GONE);
        } else {
            lblProviderDesc.setVisibility(View.VISIBLE);
            lblProviderDesc.setText("" + jObject.optString("description"));
        }
    }
                    ...
                    if (flowValue != 0) {
                        if (!source_lat.equalsIgnoreCase("") && !source_lng.equalsIgnoreCase("")) {
                            ...
                            if (PreviousStatus.equalsIgnoreCase("ACCEPTED") || PreviousStatus.equalsIgnoreCase("STARTED")) {
                                ...
                                try {
                                    String s_lat = requestStatusCheckObject.getJSONObject("provider").optString("latitude");
                                    String s_lng = requestStatusCheckObject.getJSONObject("provider").optString("longitude");
